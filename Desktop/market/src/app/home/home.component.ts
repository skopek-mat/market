...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  loadRates(): void {
    ...
    this.homeService.getCurrencyRatesFromNBP().then(rates => {
      this.currencies = [];

      for (const cur of rates) {
      ...
      for (const crypto of cryptoRates) {
        this.currencies.push({
  ...
  loadGoldData(): void {
    this.homeService.getGoldRates().then(goldPrices => {
      ...
      this.renderGoldChart(this.goldLabels, this.goldData);
    }).catch(err => {
      console.error('Błąd pobierania danych złota:', err);
    });
  }

  renderGoldChart(labels: string[], data: number[]): void {
  ...
  updateClocks(): void {
    this.newYorkTime = this.getTimeInZone('America/New_York');
    this.londonTime = this.getTimeInZone('Europe/London');
    this.tokyoTime = this.getTimeInZone('Asia/Tokyo');
  }
